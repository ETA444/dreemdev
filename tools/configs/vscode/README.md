...
## Overview of Scripts 
- `setup-vscode-extensions.sh`: all VSC extensions can be installed from here

Run from terminal (after `code` CLI is available):
...
---
## Overview of `setup-vscode-extensions.sh`
...
Here’s an updated `README.md` you can paste over the current one, with the `--force` behavior integrated and the small Jupyter fix applied.
...
- `setup-vscode-extensions.sh`: installs (and optionally updates) my curated VS Code extension stack.
...
The script passes `--force` through to `code --install-extension` when you call it with `--force`, so it doubles as a periodic “update all my core extensions” command. [web:138][web:50]
...
- `ms-python.python` and `ms-python.vscode-pylance` are the core Python + language server stack.  
- `ms-toolsai.jupyter` provides first‑class Jupyter notebook support inside VS Code. [web:161][web:127]  
- `ms-toolsai.datawrangler` adds a visual data grid and transformations, similar to GUI data tools. [web:125]  
- `ms-toolsai.python-ds-extension-pack` is Microsoft’s curated pack for Python data science work in VS Code. [web:125]
...
I can keep all installed but usually only actively “drive” one or two of them per project.
...
- `Dracula Official` is my main dark theme. [web:120][web:132]  
- `Material Icon Theme` provides clear icons per file type.  
- `Material Theme` (One Dark Pro style) and `Tokyo Night` are alternative dark schemes I like. [web:124][web:126]
```
